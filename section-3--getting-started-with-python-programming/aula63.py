"""
Calculo do segundo dígito do CPF
CPF: 746.824.890-70
Colete a soma dos 9 primeiros dígitos do CPF,
MAIS O PRIMEIRO DIGITO,
multiplicando cada um dos valores por uma
contagem regressiva começando de 11
Ex.:  746.824.890-70 (7468248907)
   11 10  9  8  7  6  5  4  3  2
*  7   4  6  8  2  4  8  9  0  7 <-- PRIMEIRO DIGITO
   77 40 54 64 14 24 40 36  0 14
Somar todos os resultados:
77+40+54+64+14+24+40+36+0+14 = 363
Multiplicar o resultado anterior por 10
363 * 10 = 3630
Obter o resto da divisão da conta anterior por 11
3630 % 11 = 0
Se o resultado anterior for maior que 9:
    resultado é 0
contrário disso:
    resultado é o valor da conta
O segundo dígito do CPF é 0
"""
import sys

cpf = input('CPF [746.824.890-70]: ')
digitos_cpf = []
for letra in cpf:
    if letra.isdigit():
        digitos_cpf.append(int(letra))

entrada_e_sequencial = [digitos_cpf[0]] * len(digitos_cpf) == digitos_cpf
if entrada_e_sequencial:
    print('Você enviou dados sequenciais.')
    sys.exit()

nove_primeiros_digitos = digitos_cpf[:9]
peso = 10
soma_ponderada = 0
for digito in nove_primeiros_digitos:
    soma_ponderada += digito * peso
    peso -= 1

primeiro_digito_cru = (soma_ponderada * 10) % 11
primeiro_digito = primeiro_digito_cru if primeiro_digito_cru <= 9 else 0

print('O primeiro dígito do CPF é', primeiro_digito)

dez_primeiros_digitos = nove_primeiros_digitos + [primeiro_digito]
peso = 11
soma_ponderada = 0
for digito in dez_primeiros_digitos:
    soma_ponderada += digito * peso
    peso -= 1

segundo_digito_cru = (soma_ponderada * 10) % 11
segundo_digito = segundo_digito_cru if segundo_digito_cru <= 9 else 0

print('O segundo dígito do CPF é', segundo_digito)

digitos_cpf_calculados = dez_primeiros_digitos + [segundo_digito]

if digitos_cpf == digitos_cpf_calculados:
    print(f'{cpf} é Valido')
else:
    print(f'{cpf} é Inválido')
